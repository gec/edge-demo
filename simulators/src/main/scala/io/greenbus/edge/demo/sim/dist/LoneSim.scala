...
import io.greenbus.edge.demo.sim.{ SimulatorActor, SimulatorContext }
...
object LoneSim {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val subscribeConfig = new SimSubscribeConfig(EndpointId(Path(prefix ++ Seq("CHP"))), EndpointId(Path(prefix ++ Seq("Load"))), EndpointId(Path(prefix ++ Seq("PV"))))

    def buildMgr(marshaller: CallMarshaller) = {
      new LoneSimMgr(marshaller, ctx, producerServices, consumerServices, subscribeConfig)
    }
